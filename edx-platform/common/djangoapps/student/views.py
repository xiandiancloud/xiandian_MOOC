...
def _do_create_account(post_vars, extended_profile=None):
    ...
    profile.school_name = post_vars.get('name_of_school')
    profile.class_name = post_vars.get('name_of_class')
    profile.major = post_vars.get('name_of_major')
    profile.admission_time = post_vars.get('year_of_admission') + '-' + post_vars.get('month_of_admission')
...
def create_account(request, post_override=None):  # pylint: disable-msg=too-many-statements
    ...
    aa = post_vars['year_of_admission'] + '-' +post_vars['month_of_admission']
    bbb = post_vars['confirmpassword']
    print bbb
    print aa
    ...
    if settings.FEATURES.get('ENFORCE_PASSWORD_POLICY', False) and not DoExternalAuth:
        try:
            ...
            if password != confirmpassword:
                js['value'] = _('the two password should be the same').format(field=a)
                js['field'] = 'password'
                return JsonResponse(js, status=400)
